  ...
  loadJSON(async function (data) {
    ...
    for (let n = 0; n < 2; n++) {
      for (let i = 0; i < 11; i++) {
        ...
        let myImage = new Image(100, 120);
        myImage.src = imgPfad;

        classifier = await mobilenet.classification(myImage, () => console.log("loading features"));
    ...
    classifier.classify(img, function (err, result) {
      ...
      if (round(result[0].confidence, 7) > round(result[1].confidence, 7))
        console.log("CANCER !!!!")
      else
        console.log("SAFE !!!!")
